...
def handle_webhook():
    try:
        ...
        if request.headers.get("X-Telegram-Bot-Api-Secret-Token") != SECRET_TOKEN:
            print("❌ Token secret invalide.")
            return Response("Unauthorized", status=403)

        # 📦 Données reçues
        update_data = request.get_json(force=True)

        # ✅ Création dynamique de l’application Telegram
        app = ApplicationBuilder().token(TOKEN).build()

        # 🧠 Association correcte du bot à l’objet Update
        ...
        print("✅ Commande Telegram exécutée avec succès.")
